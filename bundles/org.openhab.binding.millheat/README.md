...
## Migrating to new API (2023)

Mill changed a new API backend in 2023. This binding is updated to support this new API.

### Manual migration steps

1. `homeId` for Homes and `roomId` for Rooms are now UUIDs and not numbers. You can find these in the app or by using
   auto discovery.
2. `macAddress` on heaters are now in UPPERCASE with colons intact. You can find these in the app or by using auto
   discovery.
3. `heaterId` as an alternative to `macAddress` is no longer supported.
...
- `macAddress` = network mac address of device in UPPERCASE with segment colons intact, ie `FF:FF:FF:AA:BB:CC`.  
...
Thing home monaco "Penthouse Monaco"[homeId="aaaa-bbbb-cccc-uuid"] // UUID 
Thing room office "Office room"[roomId="aaaa-bbbb-cccc-uuid"] // UUID
Thing heater office "Office panel heater"[macAddress="F0:XX:XX:XX:XX:XX",power=900] // macAddress in UPPERCASE
} 
```
